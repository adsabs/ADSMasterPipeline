...
class TestSitemapWorkflow(unittest.TestCase):
    ...
    def test_sitemap_file_regeneration_after_deletion_and_cleanup(self):
        ...
        try:
            ...
            self.assertEqual(
                cleanup_result["invalid_removed"], 1, "Should remove 1 invalid record"
            )
            self.assertTrue(
                cleanup_result["files_regenerated"],
                "Should indicate files need regeneration",
            )
            self.assertEqual(
                cleanup_result["files_flagged"],
                1,
                "Should have flagged 1 file for regeneration",
            )
            ...
            with self.app.session_scope() as session:
                ...
                self.assertEqual(
                    len(final_bibcodes), 2, "Should have exactly 2 valid records"
                )
                self.assertEqual(
                    set(final_bibcodes),
                    set(remaining_bibcodes),
                    "Should match expected bibcodes",
                )

                # Invalid record should be completely removed
                invalid_count = (
                    session.query(SitemapInfo)
                    .filter_by(bibcode="2023FileRegenInvalid")
                    .count()
                )
                self.assertEqual(
                    invalid_count, 0, "Invalid record should be removed by cleanup"
                )
    ...
    def test_task_manage_sitemap_add_action_with_solr_filtering(self):
        ...
        with self.app.session_scope() as session:
            ...
            self.assertGreater(
                len(sitemap_records), 0, "Should add at least some records"
            )
            # Check that valid record is included
            valid_bibcodes = [r.bibcode for r in sitemap_records]
            self.assertIn(
                "2023Add..1..1A", valid_bibcodes, "Should include the valid record"
            )
            # Check that invalid record is not included
            self.assertNotIn(
                "2023Add..1..2B",
                valid_bibcodes,
                "Should not include the invalid record",
            )
    ...
    def test_task_manage_sitemap_add_action_batch_processing(self):
        ...
        test_bibcodes = [
            f"2023AddBatch..{i:03d}..{i:03d}A" for i in range(1, 101)
        ]  # 100 records
        ...
        original_batch_size = self.app.conf.get("SITEMAP_BOOTSTRAP_BATCH_SIZE", 50000)
        original_max_records_per_sitemap = self.app.conf.get(
        ...
        try:
            ...
            with self.app.session_scope() as session:
                ...
                self.assertGreaterEqual(
                    len(filename_set), 2, "Should use multiple sitemap files"
                )
                ...
                for filename in filename_set:
                    ...
                    self.assertLessEqual(
                        count, 25, f"File {filename} should not exceed max records"
                    )
    ...
    def test_task_manage_sitemap_force_update_action_with_solr_filtering(self):
        ...
        with self.app.session_scope() as session:
            ...
            self.assertIsNotNone(
                sitemap_record, "Record should exist after force-update attempt"
            )
            # Verify record was not updated
            self.assertEqual(
                sitemap_record.update_flag,
                False,
                "Record should not be marked for update",
            )
    ...
    def test_task_manage_sitemap_force_update_action_batch_processing(self):
        ...
        # Create a large number of test records to test batch processing
        test_bibcodes = [
            f"2023ForceBatch..{i:03d}..{i:03d}A" for i in range(1, 101)
        ]  # 100 records
        ...
        with self.app.session_scope() as session:
            ...
            for i, bibcode in enumerate(test_bibcodes):
                ...
                sitemap_record.record_id = record.id
                sitemap_record.sitemap_filename = (
                    f"sitemap_bib_{(i // 50) + 1}.xml"  # 2 files
                )
                sitemap_record.update_flag = False
                session.add(sitemap_record)
            session.commit()
        ...
        try:
            ...
            with self.app.session_scope() as session:
                ...
                our_records = (
                    session.query(Records)
                    .filter(Records.bibcode.like("2023ForceBatch%"))
                    .all()
                )
                self.assertEqual(
                    len(our_records),
                    100,
                    "All 100 test records should still be in the database",
                )
                ...
                for record in our_records:
                    ...
                    if record.status == "solr-failed":
                        self.assertEqual(
                            sitemap_record.update_flag,
                            False,
                            "Solr-failed records should not be marked for update",
                        )
                    else:
                        self.assertEqual(
                            sitemap_record.update_flag,
                            True,
                            "Success records should be marked for update",
                        )
        finally:
            # Restore original batch size
            self.app.conf["SITEMAP_BOOTSTRAP_BATCH_SIZE"] = original_batch_size
    ...
    def test_task_manage_sitemap_remove_action_batch_processing(self):
        ...
        # Create a large number of test records to test batch processing
        test_bibcodes = [
            f"2023Remove..{i:03d}..{i:03d}A" for i in range(1, 101)
        ]  # 100 records
        ...
        with self.app.session_scope() as session:
            ...
            self.assertEqual(
                remaining_count, 0, "All sitemap records should be removed"
            )
        ...
        self.assertEqual(
            files_to_delete, expected_files, "Should delete both sitemap files"
        )
        self.assertEqual(
            sitemap_dir, self.app.sitemap_dir, "Should use correct sitemap directory"
        )
    ...
    def test_task_manage_sitemap_bootstrap_with_solr_filtering(self):
        ...
        with self.app.session_scope() as session:
            ...
            self.assertEqual(
                len(our_sitemap_records), 2, "Should have 2 sitemap records"
            )
            self.assertIn(
                "2023Boot..1..1A", valid_bibcodes, "Should include first valid record"
            )
            self.assertIn(
                "2023Boot..1..3C", valid_bibcodes, "Should include second valid record"
            )
            self.assertNotIn(
                "2023Boot..1..2B",
                valid_bibcodes,
                "Should not include solr-failed record",
            )
    ...
    def test_task_manage_sitemap_bootstrap_action_batch_processing(self):
        ...
        # Create a large number of test records to test batch processing
        test_bibcodes = [
            f"2023BootBatch..{i:03d}..{i:03d}A" for i in range(1, 201)
        ]  # 200 records
        ...
        try:
            ...
            with self.app.session_scope() as session:
                ...
                self.assertIn(
                    "sitemap_bib_1.xml", filename_set, "Should have sitemap_bib_1.xml"
                )
                self.assertIn(
                    "sitemap_bib_2.xml", filename_set, "Should have sitemap_bib_2.xml"
                )
                self.assertIn(
                    "sitemap_bib_3.xml", filename_set, "Should have sitemap_bib_3.xml"
                )
                self.assertIn(
                    "sitemap_bib_4.xml", filename_set, "Should have sitemap_bib_4.xml"
                )
                ...
                for filename in filename_set:
                    ...
                    if filename == "sitemap_bib_4.xml":
                        self.assertEqual(
                            count, 30, f"File {filename} should have 50 records"
                        )
                    else:
                        self.assertEqual(
                            count, 50, f"File {filename} should have 50 records"
                        )
    ...
    def test_task_update_sitemap_files_orchestration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch(
                "adsmp.tasks.task_generate_single_sitemap.apply_async"
            ) as mock_generate:
                with patch(
                    "adsmp.tasks.task_generate_sitemap_index.apply_async"
                ) as mock_index:
                    ...
                    self.assertTrue(
                        ...
                        "Should have spawned sitemap generation tasks",
                    )
                    self.assertGreater(
                        mock_generate.call_count,
                        0,
                        "Should have at least one sitemap file to generate",
                    )

                    # Verify task_generate_sitemap_index was called
                    self.assertTrue(
                        mock_index.called, "Should have spawned index generation task"
                    )
    ...
    def test_task_update_sitemap_files_full_workflow(self):
        ...
        try:
            ...
            with self.app.session_scope() as session:
                # Clean up any existing test data
                session.query(SitemapInfo).filter(
                    SitemapInfo.bibcode.like("2023UpdateTest%")
                ).delete(synchronize_session=False)
                session.query(Records).filter(
                    Records.bibcode.like("2023UpdateTest%")
                ).delete(synchronize_session=False)
                session.commit()
            ...
            sites_config = {"ads": {"name": "ADS"}, "scix": {"name": "SciX"}}
            original_sites = self.app.conf.get("SITES")
            self.app.conf["SITES"] = sites_config
            ...
            for site_key in sites_config.keys():
                sitemap_file = os.path.join(
                    temp_dir, site_key, "sitemap_bib_update_test.xml"
                )
                expected_files.append(sitemap_file)
                self.assertTrue(
                    os.path.exists(sitemap_file),
                    f"Sitemap file should exist: {sitemap_file}",
                )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for site_key, site_config in sites_config.items():
                ...
                for filename in expected_filenames:
                    ...
                    self.assertIn(
                        f"<loc>{html.escape(expected_url)}</loc>",
                        index_content,
    ...
    def test_task_generate_sitemap_index(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for filename in sample_sitemaps:
                ...
                self.assertIn(
                    f"<loc>{html.escape(sitemap_url)}</loc>",
                    ads_index_content,
