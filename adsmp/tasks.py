...
def task_manage_sitemap(bibcodes, action):
    ...
    if action == 'delete-table':
        ...
        with app.session_scope() as session:
            ...
            max_records_per_sitemap = app.conf.get('MAX_RECORDS_PER_SITEMAP', 50000)
            processed = 0
            successful_count = 0
            failed_count = 0
            last_id = 0
                ...
                try:
                    ...
                    if action == 'add':
                        if not should_include_in_sitemap(record):
                            # Skip adding this record - don't add/update invalid records
                            logger.debug('Skipping %s for sitemap add: does not meet inclusion criteria (status: %s)', 
                                       bibcode, record.get('status'))
                            failed_count += 1
                            continue
...
def task_cleanup_invalid_sitemaps():
    ...
    batch_size = app.conf.get('SITEMAP_BOOTSTRAP_BATCH_SIZE', 10000)
    total_removed = 0
    batch_count = 0
    
    with app.session_scope() as session:
        # Get total count for progress tracking
        total_sitemap_records = session.query(SitemapInfo).count()
        logger.info('Processing %d sitemap entries for cleanup', total_sitemap_records)
        ...
        while True:
            ...
            logger.info('Processing batch %d: checking %d records (offset: %d)', 
                       batch_count + 1, len(batch_records), offset)
            
            # Find invalid records in this batch
            invalid_bibcodes = []
            ...
            for sitemap_bibcode, bib_data, bib_data_updated, solr_processed, status in batch_records:
                try:
                    ...
                    record = {
                        ...
                        'solr_processed': solr_processed,
                        'status': status
                    }
                    
                    # Check if record still meets inclusion criteria
                    if not should_include_in_sitemap(record):
                        logger.debug('Sitemap record %s no longer meets inclusion criteria (status: %s)', 
                                   sitemap_bibcode, status)
                        invalid_bibcodes.append(sitemap_bibcode)
                        
                except Exception as e:
                    logger.error('Error checking sitemap record %s: %s', sitemap_bibcode, str(e))
                    continue
            ...
            if invalid_bibcodes:
                ...
                total_removed += len(invalid_bibcodes)
                logger.info('Batch %d: removed %d invalid records', batch_count + 1, len(invalid_bibcodes))
            else:
                logger.info('Batch %d: no invalid records found', batch_count + 1)
            
            batch_count += 1
            offset += batch_size
    
    # Schedule file regeneration if any records were removed
    if total_removed > 0:
        logger.info('Cleanup completed: %d invalid records removed across %d batches', total_removed, batch_count)
        ...
        return {
            ...
            'total_records_checked': batch_count * batch_size,
            'file_regeneration_task': file_result.id
        }
    else:
        logger.info('Cleanup completed: no invalid records found across %d batches', batch_count)
        
        return {
            'removed_count': 0,
            'batches_processed': batch_count,
            'total_records_checked': batch_count * batch_size,
            'file_regeneration_task': None
        }
