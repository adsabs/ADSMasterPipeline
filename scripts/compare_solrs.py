            ...
            if doc1[key] != doc2[key]:
                if isnumber(doc1[key]) and isnumber(doc2[key]):
                    ...
                    if doc1[key].sort() != doc2[key].sort():
                        message = \
                            u'bibcode = {} array values mismatch, {}, {}'.format(bibcode,
                                doc1[key], doc2[key])
                        mismatches.append(key)
                else:
                    error_count += 1
...
def main():
    ...
    parser.add_argument('--solr-endpoints', nargs=2,
                        dest='solr_endpoints', default=[SOLR1_PATH,
                        SOLR2_PATH], help='define two solr endpoints')
